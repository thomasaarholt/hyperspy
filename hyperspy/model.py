...
class BaseModel(list):
        ...
        for comp in self:
            if comp.active:
                for parameter in comp.parameters:
                    if parameter.free:
                        if not parameter._is_linear:
                            nonlinear.append(para)  # component is not linear
                            break
        return nonlinear
    ...
    def _check_and_set_linear_parameters_not_zero(self):
        ...
        if nonlinpara:
            ...
            logging.warning('The values of the following parameters is '
                            'zero. If this is correct, ignore this'
        ...
        if self._is_multifit:
            ...
            for i, para in enumerate(self.free_parameters):
                para.map['values'] = para.map['values'] * \
                    self.coefficient_array[..., i]
                para.map['std'] = np.abs(
                    para.map['values']*standard_error_from_covariance(
                        covariance)[..., i])
            ...
            covariance = self.calculate_covariance_matrix()
            self.p_std = np.abs(
                self.p0 * standard_error_from_covariance(covariance))
